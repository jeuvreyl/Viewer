...
def prepare_test_image_in_db(test_user):
    """
    Prepare a dummy image with a thumbnail and insert them in database
    """
    test_image = UploadedImage(file_name=TEST_FILENAME, file=TEST_FILE_CONTENT, user=test_user)
...
def prepare_test_images_in_db(test_user):
    """
    Prepare a dummy image with a thumbnail and insert them in database
    """
    test_filename = "image.jpeg"
    ...
    for i in range(10):
        ...
        test_image = UploadedImage(file_name=image_name, file=file_content, user=test_user)
...
def verify_delete_in_db(image_names_list, number_expected):
...
def assert_file_in_db(saved_file, test_filename, test_file_content=None):
    ...
    if test_file_content:
        assert saved_file.file == test_file_content, 'Content mismatch for file {}'.format(saved_file.file_name)
...
class BaseTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.test_user = User.query.filter(User.username == 'test').one()
...
class ListTestCase(BaseTestCase):
    ...
    def test_paginate_list_screen(self):
        ...
        prepare_test_images_in_db(self.test_user)
        response = self.client.get(url_for("list_images"))
        assert response.get_data(), "Nothing has been returned"
        assert response.mimetype == "text/html", "Wrong mimeType returned"
        self.assert200(response, "Wrong response code")
...
class ImageTestCase(BaseTestCase):
    def test_upload(self):
        ...
        assert_file_in_db(saved_file, test_filename, test_file_content=test_file_content)

        saved_thumbnail = saved_file.thumbnail
        assert_file_in_db(saved_thumbnail, test_filename, test_file_content=None)
    ...
    def test_upload_duplicates(self):
        ...
        """
        prepare_test_image_in_db(self.test_user)
        response = self.client.post(url_for("upload"),
                                    data=dict(uploaded_images=(BytesIO(TEST_FILE_CONTENT), TEST_FILENAME)),
                                    follow_redirects=True)
    ...
    def test_delete_one_image(self):
        ...
        """
        image_name = prepare_test_image_in_db(self.test_user)
        response = self.client.post(url_for("delete_images"),
                                    data=dict(delete_ids=["1"]))
        self.assert_redirects(response, url_for('list_images'))
        verify_delete_in_db(image_name, number_expected=0)

    def test_delete_several_images(self):
        """
        Insert 10 images, delete them by request on /delete/ route and check again in DB
        """
        image_names = prepare_test_images_in_db(self.test_user)
        response = self.client.post(url_for("delete_images"),
                                    data=dict(delete_ids=[str(i) for i in range(1, 11)]))
        self.assert_redirects(response, url_for('list_images'))
        verify_delete_in_db(image_names, number_expected=0)
    ...
    def test_delete_no_image(self):
        ...
        image_name = prepare_test_image_in_db(self.test_user)
        response = self.client.post(url_for("delete_images"),
                                    data=dict(delete_ids=[]))
        self.assert_redirects(response, url_for('list_images'))
        verify_delete_in_db(image_name, number_expected=1)